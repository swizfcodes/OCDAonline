...
function toggleSidebar() {
  if (screenWidth() < 768) {
    ...
    if (
      sidebar.classList.contains("w-11") ||
...
function toggleDropdown() {
  ...
  document
    ...
    .classList.toggle("hidden", type !== "ward");
  document
    .getElementById("quartersSelect")
    .classList.toggle("hidden", type !== "quarters");
...
async function fetchMonthlyTotal() {
  ...
  try {
    ...
    const res = await fetch(
      `/api/member/ledger-entry/monthly-total?month=${year}-${month}&phoneno=${encodeURIComponent(phoneno)}`,
    );
    if (!res.ok) {
...
function enableEditMode() {
  document
    .querySelectorAll(
      "#profileSection select, #profileSection input:not([id=phone]):not([id=age]):not([id=exitDate])",
    )
...
function cancelEdit() {
  document
    .querySelectorAll("#profileSection select, #profileSection input")
...
async function updateProfile(event) {
  ...
  fields.forEach((field) => {
    const input =
      ...
      form.querySelector(`#${field.toLowerCase()}`) ||
      form.querySelector(`#${field.toUpperCase()}`);
...
function clearReceipts() {
  ...
  document.getElementById("receiptTable").innerHTML = "";
  document.getElementById("enquiryResults").innerHTML =
    "<p>Select a ward or quarters to begin.</p>"; // <-- also clear enquiry result container
...
async function toggleEnquiry() {
  ...
  wardSelect.innerHTML = wards
    ...
    .join("");
  quartersSelect.innerHTML = quarters
    .map((q) => `<option value="${q}">${q}</option>`)
    .join("");

  wardSelect.onchange = () => fetchEnquiry("ward", wardSelect.value);
  quartersSelect.onchange = () =>
    fetchEnquiry("quarters", quartersSelect.value);
}
...
async function fetchEnquiry(type, value) {
  ...
  try {
    ...
    const res = await fetch(
      `/api/enquiry/${type}/${encodeURIComponent(value)}?${queryParams.toString()}`,
    );
    const data = await res.json();
    ...
    document.getElementById("enquiryResults").innerHTML =
      '<p class="text-red-500">Error fetching records.</p>';
  }
}
